...
CharacterizeVariants_single_input <- function(path_to_output) {
  ...
  names(vcf)<-c('chrom', 'chromEnd', 'ref', 'alt', 'info')
  #format chr column
  if(grepl('chr', vcf$chrom[1], fixed=TRUE)==FALSE) {
    vcf[, chrom := paste('chr', chrom, sep='')]
  }
  #keep only SNVs
  if( nchar(ref)>1 | nchar(alt)>1 ) stop('variant or reference is not single nucleotide')
  vcf[, chromStart := as.numeric(chromEnd)-1]
  ...
  if( nrow(variants)==0 ) stop('surrounding hg38 fasta contains an "N" base call')
  ...
  for (n in 1:length(unique_vars)) {
    ...
    suppressWarnings(matches[, c('miR_info', 'gwas_info', 'clinvar_info') := NULL])
